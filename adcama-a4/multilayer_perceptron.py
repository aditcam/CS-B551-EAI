...
class MultilayerPerceptron:
    ...
    def _initialize(self, X, y):
        ...
        output_label_cnt = np.max(y)
        self._X = X
        self._y = one_hot_encoding(y)
        self._h_weights = np.random.rand(feature_count,self.n_hidden)
        self._h_bias = np.random.randn(self.n_hidden)
        self._o_weights = np.random.rand(self.n_hidden,output_label_cnt+1)
        self._o_bias = np.random.randn(output_label_cnt+1)
    ...
    def fit(self, X, y):
        ...
        # Reference : https://stackabuse.com/creating-a-neural-network-from-scratch-in-python-multi-class-classification
        ...
        for i in range(self.n_iterations):
            ...
            # Feed Forward step
            # Basically performs [activation_function(input * weights + bias)]
            op_hidden = np.dot(self._X,self._h_weights)+self._h_bias
            op_hidden_a = self.hidden_activation(op_hidden) # apply activation function 

            op_output = np.dot(op_hidden_a,self._o_weights)+self._o_bias
            op_output_a = self._output_activation(op_output) # apply activation function 

            # Back Propgation Step (Incomplete --> Still pending)
            # Calculate the required gradients

            error = op_output_a - self._y
            derivative_op = self._output_activation(op_output,derivative=True)
            derivative_hidden = self._output_activation(op_hidden,derivative=True)
            error_hidden = np.dot(op_output_a,self._o_weights.T)

            delta_output = error * derivative_op
            delta_hidden = error_hidden * derivative_hidden

            # Updating the weights 
            
            self._h_weights = self._h_weights - (self.learning_rate * np.dot(delta_hidden.T,self._X))/len(self._X)
            self._o_weights = self._o_weights - (self.learning_rate * np.dot(delta_output.T,op_hidden_a))/len(self._X)
    ...
    def predict(self, X):
        ...
        """
        # Here we apply the updated weights and biases calculated from the fit function and get the predictions

        op_hidden = np.dot(self._X,self._h_weights)+self._h_bias
        op_hidden_a = self.hidden_activation(op_hidden) # apply activation function 

        op_output = np.dot(op_hidden,self._o_weights)+self._o_bias
        op_output_a = self._output_activation(op_output) # apply activation function 

        result = []

        for x in op_output_a:
